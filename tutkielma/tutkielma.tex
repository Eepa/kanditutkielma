...
\textbf{historiasta ehkä lisää ja tutkimuksesta ja lisäksi käsittele ne hypoteesit mutaatiotestauksen takana eli competent programmer hypothesis ja coupling effect}

\subsection{Perinteinen mutaatiotestausprosessi}

Tavoitteena on havaita testien avulla lähdekoodiin tehdyt muutokset~\cite[s. 35]{Offutt:Untch:2001}.
...
Seuraavaksi testit suoritetaan jokaisen elävän mutantin kohdalla~\cite[s. 35]{Offutt:Untch:2001}. Mutaatiotestausprosessin työvaiheita \textbf{(havainnollistetaan?)}/esitellään kuvassa \ref{figure:Mutaatiotestausprosessi}. Sinisillä laatikoilla kuvataan työvaiheita. Työvaiheiden aikana käytettäviä resursseja kuvataan vihreillä ja keltaisilla laatikoilla.
...
\textbf{Tästä välistä puuttuu ekvivalenttien mutanttien tarkastelu ja testien korjaus+lisääminen tarvittaessa.}
...
\textbf{Lisää juttua siitä, että mutaatiotestaus kasvattaa testitkattavuutta. Ja vois olla kiva joku kuva mutaatiotestausprosessista --> että näkisi visuaalisesti sen silmukan, mitä siinä tehdään.}

\textbf{\textit{Kuvasta puuttuu valmista-kohdasta mutaatiopistemäärä.}}
...
\textbf{Omaa muotoilua perustuen Uniting orthogonal 2000 artikkeliin --> uusi mutaatiotestausprosessi:} Kuvasta \ref{figure:Mutaatiotestausprosessi} huomataan, että mutaatiotestausprosessi sisältää silmukoita. Ensimmäinen silmukka sisältää mutanttien generoinnin, testien suorituksen alkuperäiselle ohjelmalle ja testien tulosten tarkastamisen jälkeen virheiden korjauksen. Näistä työvaiheista testien tulosten tarkastaminen ja alkuperäisen ohjelman virheiden korjaaminen vaativat ihmisten työpanosta \textbf{(human intensive? käännös)}.

Toinen silmukka eli mutaatiotestauksen pääsilmukka muodostuu työvaiheista, jotka suoritetaan testien läpimenotarkastuksen jälkeen. Toiseen silmukkaan sisältyvät myös ensimmäisen silmukan työvaiheista testien suoritus lähdekoodille ja testien läpimenotarkastus. Tämä tarkoittaa, että silmukat risteävät näiden työvaiheiden kohdalla. Toisessa silmukassa manuaalista työtä vaativat ekvivalenttien mutanttien etsiminen ja merkitseminen ja uusien testien lisääminen tai olemassa olevien testien kehittäminen.

\textbf{Onko konditionaali sallittu --> en tiedä, onko tämä jo käytössä?}. Jotta mutaatiotestausprosessia voidaan tehostaa, manuaaliset työvaiheet tulisi poistaa mutaatiotestauksen pääsilmukasta~\cite[s. 41]{Offutt:Untch:2001}. Perinteisen mutaatiotestausprosessin tilalle tulisi uusi testausprosessi, jossa osa työläistä manuaalisista työvaiheista suoritettaisiin automaattisesti. Ekvivalenttien mutanttien tunnistaminen ja testien generointi ja kehittäminen voitaisiin uudessa testausprosessissa automatisoida, mikä tehostaisi mutaatiotestausprosessia. Ainoa merkittävä manuaalinen työvaihe uudessa mutaatiotestausprosessissa olisi tarkistaa, ovatko alkuperäiselle lähdekoodille suoritetut testit menneet läpi.

\section{Mutaatiotestaus oliojärjestelmissä}

Esittelyteksti kappaleelle tähän.
...
\subsection{Mutaatiosovellukset oliojärjestelmille}
...
\textbf{Ratkaisematon ongelma. Esimerkki ekvivalenteista mutanteista. Miten on yritetty ratkaista? --> erilaisilla tunnistusalgoritmeilla, tulevaisuudessa välttää ekvivalenttien mutanttien syntymistä.}
...
\textbf{Laitteisto, testien määrä-->suoritus tehotonta. Miten on yritetty ratkaista --> weak mutation, suorituksen jakaminen usealle prosessorille, mutanttien määrän vähentäminen}
...
\subsection{Ihmisten työpanosongelma}
