...
\textbf{Lisää tähän? Binderin kirjasta löyty tietoa. Ja joistakin artikkeleista. Voisi käydä myös läpi fault - error - failure osat.}
...
MP = \frac{T}{K - E}
\end{equation}
missä \textit{T} on tapettujen mutanttien määrä, \textit{K} on kaikkien mutanttien määrä ja \textit{E} on ekvivalenttien mutanttien määrä. Mutaatiopistemäärän maksimiarvo 1 saavutetaan, kun testeillä saadaan tapettua kaikki mutantit~\cite[s. 36]{Offutt:Untch:2001}. Testit ovat tällöin riittävät havaitsemaan kaikki mutantit \textit{\textbf{adequate relative to the mutants}}. Perinteinen mutaatiotestausprosessi päättyy, kun kaikki \textbf{paitsi ekvivalentit/ei-ekvivalentit} mutantit on tapettu. Tämä on merkitty kuvaan \ref{figure:Mutaatiotestausprosessi} violetilla laatikolla.
...
Mutaatiotestausprosessin suoritus voi edetä silmukoissa, joissa työvaiheita toistetaan kunnes prosessissa määritellyt ehdot täyttyvät. Perinteisessä mutaatiotestausprosessissa ensimmäinen ehto on, että alkuperäiselle lähdekoodille suoritetut testit menevät läpi ennen kuin mutaatiotestausta jatketaan. Toinen ehto on, että kaikki paitsi ekvivalentit mutantit on tapettava ennen kuin prosessi voi päättyä. Jos ehdot eivät täyty, toistetaan työvaiheita sykleittäin kunnes prosessi saadaan päättymään. Ehtojen tarkastusvaiheet on merkitty kuvaan \ref{figure:Mutaatiotestausprosessi} vinoneliön muotoisilla laatikoilla.

Kuvasta \ref{figure:Mutaatiotestausprosessi} huomataan, että perinteiseen mutaatiotestausprosessiin muodostuu kaksi silmukkaa. Ensimmäiseen silmukkaan sisältyy mutanttien generointi, testien suoritus alkuperäiselle lähdekoodille, testien tulosten tarkastaminen ja lähdekoodin vikojen korjaaminen. Mutaatiotestauksen pääsilmukkaan sisältyy testien suoritus lähdekoodille ja eläville mutanteille, testien tulosten tarkastus ja vertailu, ekvivalenttien mutanttien etsiminen ja merkitseminen ja testien kehittäminen.

Jotta perinteistä mutaatiotestausprosessia voidaan tehostaa, manuaaliset työvaiheet on poistettava mutaatiotestausprosessin pääsilmukasta~\cite[s. 41]{Offutt:Untch:2001}. Tällöin perinteisen mutaatiotestausprosessin tilalle muodostuu uusi testausprosessi, jossa osa työläistä manuaalisista työvaiheista suoritetaan automaattisesti. 

\textbf{En tiedä, onko tämä uusi prosessi jo käytössä?} Kuvasta \ref{figure:Mutaatiotestausprosessi} huomataan, että perinteinen mutaatiotestausprosessi sisältää silmukoita. Ensimmäinen silmukka sisältää mutanttien generoinnin, testien suorituksen alkuperäiselle lähdekoodille ja testien tulosten tarkastamisen jälkeen virheiden korjauksen \textbf{(vs. korjataan lähdekoodista viat jotka aiheuttivat virheet)}. Näistä työvaiheista alkuperäisten testitulosten tarkastaminen ja lähdekoodissa esiintyvien vikojen korjaaminen vaativat manuaalista työtä.
...
Kolmas mutaatiotestauksen ongelma-alue liittyy perinteiseen mutaatiotestausprosessiin sisältyvään manuaaliseen työhön. Perinteisessä mutaatiotestausprosessissa useaan prosessin työvaiheeseen sisältyy manuaalista työtä \textbf{(tätä käsittelin kappaleessa 3.3 Uusi mutaatiotestausprosessi) --> pitääkö se käsitellä uudestaan tässä kohtaa?}. Kuvassa \ref{figure:Mutaatiotestausprosessi} (sivulla \pageref{figure:Mutaatiotestausprosessi}) on esitelty perinteinen mutaatiotestausprosessi. Kuvassa esiteltyyn kaavioon on merkitty työvaihe, jossa alkuperäisten testien suorituksen jälkeen tarkastetaan, ovatko testit menneet läpi. 
...
Uusien, mutantteja tappavien testien kehittäminen on myös yksi perinteisen mutaatiotestausprosessin työläistä manuaalisista työvaiheista~\cite[s. 39]{Offutt:Untch:2001}. Mutaatiotestauksessa tavoitteena on kehittää testejä, joiden avulla saadaan tapettua mahdollisimman paljon mutantteja (mutation adequate). Manuaalisesti tällaisten testien kehittäminen on työlästä. Lisäksi sopivien syötteiden löytäminen testejä varten on yksi vaikeimmista testauksen vaiheista~\cite[s. 39]{Offutt:Untch:2001}. Testien automaattista generointia varten on kuitenkin kehitetty menetelmiä, joiden avulla testien generointi voidaan mutaatiotestausprosessissa automatisoida osittain.
